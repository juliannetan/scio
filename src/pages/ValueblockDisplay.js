...
const ValueblockPage = () => {
  ...
  function displayValueblocks(valueblockID) {
    valueblocks.map((valueblock) => {
      if (valueblock.ID == valueblockID) {
        setValueblock2({
          ID: valueblock.ID,
          created_at: valueblock.created_at,
  ...
  async function updateValueblocks(valueblockID) {
    const { data, error } = await supabase
      ...
      .update({
        ID: valueblock2.ID,
        created_at: valueblock2.created_at,
        VDQ1: valueblock2.VDQ1,
        VDQ2: valueblock2.VDQ2,
        VDQ3: valueblock2.VDQ3,
        VDQ4: valueblock2.VDQ4,
      })
          ...
          {valueblocks.map((valueblock) => (
            <tr key={valueblock.ID}>
              ...
              <td>
                <button
                  onClick={() => {
                    if (
                      ...
                      deleteValueblocks(valueblock.ID)
                  }}
                >
                  Delete
                </button>
                <button
                  onClick={() => {
                    displayValueblocks(valueblock.ID)
                  }}
                >
                  Edit
                </button>
