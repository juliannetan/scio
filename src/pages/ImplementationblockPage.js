...
const ImplementationblockPage = ({ generatedId, providedId, setNextPage }) => {
  ...
  async function uploadImage1(e) {
    ...
    const { data, error } = await supabase.storage
      .from('images/scio/' + providedId + '/ip' + '/file1')
      .upload('/' + uuidv4(), file)
  ...
  async function deleteImage1(imageName) {
    const { error } = await supabase.storage
      .from('images')
      .remove(['scio/' + providedId + '/ip' + '/file1' + '/' + imageName])
   ...
   async function uploadImage2(e) {
     ...
     const { data, error } = await supabase.storage
       .from('images/scio/' + providedId + '/ip' + '/file2')
       .upload('/' + uuidv4(), file)
   ...
   async function deleteImage2(imageName) {
     const { error } = await supabase.storage
       .from('images')
       .remove(['scio/' + providedId + '/ip' +  '/file2' + '/' + imageName])
        ...
        <Section>
          ...
          <Grid container spacing={2}>
            {images1.map((image) => (
              <Grid
                item
                key={CDNURL + providedId + '/ip' + '/file1' + '/' + image.name}
              >
                <Card>
                  <CardMedia
                    component='img'
                    height='150'
                    image={CDNURL + providedId + '/ip' + '/file1' + '/'  + image.name}
                  />
                  <CardContent>
                    <Button
                      size='small'
          ...
          <Grid container spacing={2}>
            {images2.map((image) => (
              <Grid
                item
                key={CDNURL + providedId + '/ip' + '/file2' + '/' + image.name}
              >
                <Card>
                  <CardMedia
                    component='img'
                    height='150'
                    image={CDNURL + providedId + '/ip' + '/file2' + '/'  + image.name}
                  />
                  <CardContent>
                    <Button
                      size='small'
